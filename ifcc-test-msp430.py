...
def get_ret_address(elf_file):
    """Trouve l'adresse du dernier ret de main via objdump"""
    ...
    for line in lines:
        ...
        if in_main:
            # Detect next real function (not a BB label like <BB0_exit>)
...
def run_in_mspdebug(elf_file):
    ...
    if ret_addr is None:
        if args.debug:
            print("  [debug] impossible de trouver ret dans main")
        return None
    ...
    try:
        ...
        if args.debug >= 1:
            print("  [debug] mspdebug timeout after 3 seconds")
        process.kill()
    ...
    # Try multiple R15 patterns (mspdebug outputs "(R15: 0002a)" format)
    for pattern in [
        r'\(?\s*R15\s*:\s*([0-9a-fA-F]+)',
        r'R15\s*=\s*0x([0-9a-fA-F]+)',
        r'R15\s+0x([0-9a-fA-F]+)',
    ]:
    ...
    if args.debug >= 1:
        ...
        print(f"  [debug] Last 500 chars: {stdout[-500:]}")
    return None
